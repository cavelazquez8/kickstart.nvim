...
-- Modify the close_obsidian function
local function close_obsidian()
  -- Try different methods to close Obsidian
  local methods = {
    'pkill -f obsidian',
    'am force-stop md.obsidian', -- Android specific
    'killall obsidian',
  }

  for _, method in ipairs(methods) do
    local success = os.execute(method)
    if success then
  ...
  print 'Failed to close Obsidian. You may need to close it manually.'
end
...
function AddObsidianVault(name, path)
  ...
  if not ensure_directory_exists(temp_dir) then
    print 'Failed to create temp directory for vault'
    return
  end
...
-- Modify just the open_in_obsidian function, rest remains the same
local function open_in_obsidian()
  ...
  if containing_vault then
    ...
    -- Remove any leading slash from relative_path
    ...
    -- File is outside any known vault - create a copy in the first vault instead of symlink
    local default_vault = vaults[1]
    local file_basename = vim.fn.fnamemodify(filepath, ':t')
    local temp_dir = default_vault.path .. '_temp_preview'
    local temp_file_path = temp_dir .. '/' .. file_basename
    ...
    -- Instead of symlink, copy the file
    local copy_cmd = string.format("cp '%s' '%s'", filepath, temp_file_path)
    if os.execute(copy_cmd) then
      -- Register autocmd to clean up the copied file when the buffer is closed
      vim.api.nvim_create_autocmd({ 'BufDelete', 'BufWipeout' }, {
        buffer = vim.api.nvim_get_current_buf(),
        callback = function()
          os.execute("rm -f '" .. temp_file_path .. "'")
        end,
      })

      -- Open in default vault, using the correct path
      local encoded_path = url_encode(file_basename)
      local uri = 'obsidian://open?vault=' .. default_vault.name .. '&file=' .. encoded_path

      print('Opening external file in default Obsidian vault: ' .. file_basename)
      vim.fn.jobstart({ 'xdg-open', uri }, { detach = true })
    else
...
-- Update the VimLeave autocmd to use the same closing methods
...
-- Configuration file pat-- [[ Basic Autocommands ]]
      ...
      require('mason-lspconfig').setup {
        handlers = {
          function(server_name)
            ...
            -- ertain features of an LSP (for example, turning off formatting for ts_ls)
